# tests/test_email_processor.py
"""
Unit tests for the email processor module.
"""

import os
import sys
import unittest
from datetime import datetime

# Add parent directory to path for imports
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))

from email_processor import (extract_entities, extract_dates, format_email_for_ai,
                            generate_email_response_template, analyze_sentiment,
                            extract_action_items)


class TestEmailProcessor(unittest.TestCase):
    """Test suite for email processor functions."""
    
    def test_extract_entities(self):
        """Test extracting entities from email text."""
        # Sample email body with entities
        email_body = """
        Hello team,
        
        Please contact john.doe@example.com or visit https://example.com for more information.
        We have a meeting scheduled on 05/15/2025 and another one on June 20, 2025.
        
        Regards,
        Jane Smith
        """
        
        # Extract entities
        entities = extract_entities(email_body)
        
        # Verify emails
        self.assertIn("john.doe@example.com", entities["emails"])
        
        # Verify URLs
        self.assertIn("https://example.com", entities["urls"])
        
        # Verify dates
        self.assertTrue(any("05/15/2025" in date for date in entities["dates"]))
        self.assertTrue(any("June 20, 2025" in date for date in entities["dates"]))
# tests/test_email_processor.py (continued)

    def test_extract_dates(self):
        """Test extracting dates from text."""
        # Text with various date formats
        text = """
        Meeting on 01/15/2025
        Conference call on March 20, 2025
        Deadline is 2025-04-30
        Review session on 5 April 2025
        """
        
        dates = extract_dates(text)
        
        # Verify we found all four dates
        self.assertEqual(len(dates), 4)
        self.assertTrue(any("01/15/2025" in date for date in dates))
        self.assertTrue(any("March 20, 2025" in date for date in dates))
        self.assertTrue(any("2025-04-30" in date for date in dates))
        self.assertTrue(any("5 April 2025" in date for date in dates))
    
    def test_format_email_for_ai(self):
        """Test formatting email data for AI processing."""
        # Sample email data
        email_data = {
            "subject": "Project Update",
            "body": "Hello team,\nProject is on track. Please contact john@example.com.",
            "sender": "manager@company.com",
            "date": "2025-04-25"
        }
        
        # Format for AI
        formatted = format_email_for_ai(email_data)
        
        # Verify structure
        self.assertIn("EMAIL INFORMATION:", formatted)
        self.assertIn("From: manager@company.com", formatted)
        self.assertIn("Subject: Project Update", formatted)
        self.assertIn("Hello team,", formatted)
        
        # Verify extracted entities
        self.assertIn("EXTRACTED ENTITIES:", formatted)
        self.assertIn("john@example.com", formatted)
    
    def test_generate_email_response_template(self):
        """Test generating email response template."""
        # Original email
        original_email = {
            "subject": "Question about project",
            "body": "When will the project be completed?",
            "sender": "client@example.com",
            "date": "2025-04-24"
        }
        
        # AI response
        ai_response = "The project is scheduled to be completed by next Friday."
        
        # Generate template
        template = generate_email_response_template(original_email, ai_response)
        
        # Verify template structure
        self.assertEqual(template["to"], "client@example.com")
        self.assertEqual(template["subject"], "Re: Question about project")
        self.assertIn("The project is scheduled to be completed by next Friday", template["body"])
        self.assertIn("On 2025-04-24", template["body"])
        self.assertIn("> When will the project be completed?", template["body"])
    
    def test_analyze_sentiment(self):
        """Test sentiment analysis function."""
        # Positive email
        positive_text = "Thank you for your excellent work. I really appreciate your help."
        self.assertEqual(analyze_sentiment(positive_text), "positive")
        
        # Negative email
        negative_text = "There's a problem with the service. I'm disappointed with the results."
        self.assertEqual(analyze_sentiment(negative_text), "negative")
        
        # Neutral email
        neutral_text = "Please find attached the requested documents."
        self.assertEqual(analyze_sentiment(neutral_text), "neutral")
    
    def test_extract_action_items(self):
        """Test extracting action items from email."""
        # Email with action items
        email_text = """
        Hello team,
        
        Please review the attached document by tomorrow.
        We need to finalize the report ASAP.
        Could you update the presentation with the latest figures?
        
        Let me know if you have any questions.
        """
        
        action_items = extract_action_items(email_text)
        
        # Verify we found at least 3 action items
        self.assertGreaterEqual(len(action_items), 3)
        
        # Check for specific action items
        self.assertTrue(any("review the attached document" in item.lower() for item in action_items))
        self.assertTrue(any("finalize the report" in item.lower() for item in action_items))
        self.assertTrue(any("update the presentation" in item.lower() for item in action_items))


if __name__ == '__main__':
    unittest.main()